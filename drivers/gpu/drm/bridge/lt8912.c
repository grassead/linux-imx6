...
static inline int lt8912_read(struct lt8912 *lt, u8 idx, u8 reg, u8* val) {
	return regmap_read(lt->regmap[idx], reg, val);
}
...
static int lt8912_video_setup(struct lt8912* lt) {
	...
	if (vactive == 480)
		...
		dev_err(lt->dev, "%s: unsupported yres = %d\n", __func__, vactive);
		return -1;
	}
...
static int lt8912_bridge_detach(struct drm_bridge *bridge)
{
	struct lt8912 *lt = bridge_to_lt8912(bridge);
